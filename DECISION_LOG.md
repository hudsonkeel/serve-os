...
### Decision
Ran a live Bedrock benchmark against the real Bedrock Converse API using dedicated dev AWS credentials (`serve-bedrock-dev`) — synthetic data only, no PHI, no merge. Left the pricing engine's architecture as-is (assessed, not redesigned) after a separate clarification that AI-usage cost and Serve's client-facing care pricing are unrelated pricing surfaces and must not be conflated.

### Reason
The Bedrock adapter had never been exercised against real AWS (§9/§13 of `BEDROCK_CLAUDE_PROVIDER.md`) — the fundamental unknown blocking PHI readiness was "does this code actually work against live Bedrock at all," which only a real credentialed call could answer. Separately, a question arose about whether AI could ever influence Serve's actual care/service pricing (a different concern from what Serve pays OpenAI/AWS per token) — worth answering directly rather than assuming, but redesigning pricing was explicitly out of scope for this phase.

### Result
3 newly authored synthetic cases (the original §9 harness files were session-scratch, confirmed unrecoverable) ran successfully live: 3/3 valid JSON, real token usage measured (5,412 in / 1,430 out across the 3 cases), real latency measured (avg 4,844ms/case), zero hallucinated or unsupported facts, and one confirmed security property (the least-privilege IAM policy correctly denied an out-of-scope `GetModelInvocationLoggingConfiguration` call). Two real epistemic-behavior findings were recorded (a hedged negative misread as `confirmed_no`; an ambiguous prompt rule resolved differently by Claude vs. the earlier OpenAI run) — not code defects, but worth a larger benchmark before treating Bedrock's extraction quality as at parity with OpenAI's. No code was changed (structured-output reliability was already 3/3; no concrete need to strengthen it). `tsc --noEmit` and the full `test:assessmentIntelligence` suite (mocked, zero AWS calls) both still pass. Separately, `pricingEngine.ts`/`pricingCatalog.ts` were read and documented (not modified): pricing is hardcoded, versioned only as a single current-value string persisted per-decision (no historical catalog table), and AI cannot influence numeric price — both because `recommendPricing()` takes no provider/AI-generated value and because contested facts are blocked from approval before they could ever reach pricing. Full detail, all real numbers, and the exact remaining PHI/merge blockers are recorded in `docs/architecture/BEDROCK_CLAUDE_PROVIDER.md` §9a/§10/§14/§15. `PHI READY: NO` still stands — the reason shifted from "never tested" to "deployment credentials and human sign-off still pending." Not merged to `main`.

---

### Decision
Built the native Serve OS assessment capture pipeline end to end (MediaRecorder chunked capture -> IndexedDB durability -> signed-URL upload -> server-side ffmpeg assembly -> AWS S3 staging -> one Amazon Transcribe batch job per assessment -> persisted transcript -> Bedrock extraction -> human review) and the production-shaped background orchestration to run it (a Netlify dispatcher/background-worker split, session-scoped idempotent state machine, admin-only synthetic-test authorization). Reconciled this work onto `feature/assessment-aws-transcription-pipeline`, branched fresh from `origin/main`, after `origin/main` advanced substantially (Client Readiness / Audit Readiness / Multi-Community / AxisCare Canonical Sync) past this work's original branch point.

### Reason
Proving this pipeline against a synthetic assessment, without weakening the AWS PHI gate, is the milestone that unblocks real AxisCare write-back work — see the separate AxisCare write-policy proposal for that follow-on scope, deliberately not started here. The branch reconciliation was necessary because the original branch's committed history had already merged to `main`, but a large body of uncommitted pipeline work had not — continuing on the stale branch would have silently dropped `community_id`, Client Readiness evidence recording, and a real component layout fix that landed on `main` in the interim.

### Result
Every file this pipeline touches that `origin/main` also independently changed was reconciled by hand, not overwritten in either direction — `AssessmentCaptureButton.tsx` keeps the native capture `Link` behavior and adopts `main`'s `label` prop and `display:contents` fix; `lib/actions/assessmentIntelligence.ts` and `lib/data/assessmentIntelligence.ts` keep `main`'s `community_id` resolution and Client Readiness evidence recording, with `createNativeAssessmentSession`/`createSyntheticAssessmentSession` extended to resolve and persist `community_id` themselves (previously a real gap — assessment sessions from this pipeline would otherwise have been created with no community, inconsistent with every other assessment-creation path). One migration filename collision (`20260902060000`, independently claimed by `main`'s `add_audit_session_corrections.sql`) was resolved by renumbering to `20260902370000`, immediately after `main`'s latest migration at reconciliation time. A separate, unrelated body of uncommitted work — AxisCare write-preview architecture (`reassessmentComparison.ts`, ADL/responsible-party/client-candidate mapping, 2 domain-registry HIPAA/POA fields, the `assessment_type` migration) — was deliberately left off this branch as out of scope for this milestone; it remains recoverable from the `feature/bedrock-claude-provider-abstraction` snapshot commit.