...
### Decision
Built the complete synthetic dry-run architecture bridging Serve's assessment intelligence layer to AxisCare — approved-fact mapping, new-client candidate generation, reassessment change-detection, responsible-party mapping, ADL mapping, and a composed write-safety preview — stopping deliberately at the write-transport boundary. Did not add any AxisCare write capability (`axisCareGet()` remains GET-only by construction) and did not reverse the standing read-only integration policy.

### Reason
The read-only AxisCare integration and the assessment-intelligence approval pipeline already existed independently, but nothing connected them — every approved assessment still required full manual re-entry into AxisCare. Building the connecting architecture first, provably correct via tests against real AxisCare reference data (live class/ADL catalog fetches, read-only), lets a future write adapter consume an already-validated candidate payload without redesigning the domain model — per explicit instruction, that adapter itself was deliberately not built, pending a separate authorization decision on the proposed policy amendment.

### Result
New: `lib/assessmentIntelligence/reassessmentComparison.ts` (the reassessment diff engine — a real gap; the existing `detectAndRecordConflicts()` only ever compared facts within one session), 2 new domain-registry fields for explicit-only HIPAA/medical-decision authority, `lib/integrations/axiscare/{clientClasses,clientWritePayloadTypes,clientCandidateMapping,responsiblePartyMapping,adlMapping,fullWritePreview}.ts`, `getAllConfiguredAdls()` added to `adls.ts`, and a purely additive `assessment_type` migration (written, **not yet applied to any database**). 60 new unit/integration tests, all passing; full existing suite (`test:assessmentIntelligence`, `test:axiscare`), typecheck, lint, and production build all still pass with zero regressions. A genuine domain-model limitation was found by the tests, not assumed in advance: a plain boolean field can't represent a reassessment's *severity* change, only a yes/no flip. Full architecture, the Phase 12 field matrix (exactly what's automatable vs. still-manual), and the Phase 14 security update are recorded in `docs/architecture/AXISCARE_ASSESSMENT_INTEGRATION.md`; the requested write-policy amendment proposal is in `docs/integrations/AXISCARE_WRITE_POLICY_PROPOSAL.md` (proposal only — `AXISCARE_READ_ONLY_INTEGRATION.md` remains authoritative). Not merged to `main`. No AxisCare write — real or dry-run-executed — was performed.