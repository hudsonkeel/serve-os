...
export async function getOrStartNativeCaptureSession(residentId: string): Promise<{ session?: NativeCaptureSession; error?: string }> {
  ...
  // lib/actions/assessmentIntelligence.ts) — native capture is always an existing-person flow,
  // so the linked resident's own community is the strongest source, same priority order.
  ...
  const { session, error } = await createNativeAssessmentSession({
    residentId,
    startedBy: actor.label,
