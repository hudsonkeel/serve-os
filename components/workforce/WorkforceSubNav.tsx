...
// experience spanning candidate lifecycle through active/inactive/
// terminated employment, without merging the underlying recruiting_leads
// and workforce_members data models. "Candidates" links to the existing,
// unmodified /recruiting route rather than a new page — recruiting keeps
// its own already-committed, fully working experience; Workforce simply
// becomes the coherent place operations lands to reach it. Onboarding/
// Active/Inactive/Terminated reuse the roster's own existing lifecycle
// filters (no new query logic); Identity Review reuses the existing route.
const WORKFORCE_SUB_NAV: Array<{ label: string; href: string }> = [
  { label: "Overview", href: "/workforce" },
  { label: "Candidates", href: "/recruiting" },
...
export function WorkforceSubNav({ active }: { active: "overview" | "identity-review" | null }) {
  return (
    <nav aria-label="Workforce" className="mb-6 flex flex-wrap gap-1.5 border-b border-ivory-border pb-4">
      {WORKFORCE_SUB_NAV.map((item) => {
        ...
        const isActive = (active === "overview" && isOverview) || (active === "identity-review" && isIdentityReview);
        return (
          <Link
            key={item.label}
            href={item.href}
