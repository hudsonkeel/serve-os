...
// Infection Lifecycle & Learning Loop v0.1 — structurally mirrors
// components/incidents/CreateIncidentCorrectiveActionForm.tsx exactly
// (same shared compliance_corrective_actions/corrective_action_effectiveness_reviews
// tables, same plain-insert RPC discipline), but framed around a Serve
// process / infection-control / service-delivery concern, never the
// client's infection itself — effectiveness review here, when required,
// always evaluates whether THIS corrective action worked. Deliberately
// independent of the follow-up loop (see create_infection_corrective_action's
// own comment): no follow_up_required gate.
export function CreateInfectionCorrectiveActionForm({ infectionId, onDone }: { infectionId: string; onDone?: () => void }) {
    ...
    <form onSubmit={handleSubmit} className="space-y-4 rounded-lg border border-ivory-border bg-ivory-warm p-4">
      <label className="block">
        ...
        <input
          ...
          placeholder="e.g. Reinforce PPE protocol with assigned caregiver"
      ...
      <label className="block">
        ...
        <AutoGrowTextarea
          ...
          minRows={2}
          placeholder="What Serve process, infection-control, or service-delivery concern was identified?"
        />
      </label>
      ...
      <div className="border-t border-ivory-border pt-4">
        <label className="flex items-center gap-2 font-sans text-sm text-body">
          <input type="checkbox" checked={effectivenessRequired} onChange={(e) => setEffectivenessRequired(e.target.checked)} />
