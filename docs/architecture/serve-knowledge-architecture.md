...
| Operational Readiness & Audit Preparedness Roadmap | `serve-os/documents/` (in-repo, untracked) | Architecture | Architecture | Yes | `docs/architecture/` | Needs Migration | **The originating charter for this entire architecture series.** Its stated vision — Regulations → Operational Requirements → Evidence Collection → Serve OS → Operational Readiness → Audit Preparedness → Operational Intelligence — is the direct ancestor of the Dependency Graph in the Governance Crosswalk. Recommended as the first document formally migrated. |
| Background Eligibility Policy v0.1 Draft (.docx) | `serve-os/public/` (in-repo, untracked) | Policy Manual (draft) | Organizational Knowledge | Historical | `docs/organizational-knowledge/archive/` | Superseded | Original seed for Background Eligibility Module 1; superseded in place per that module's own decision log. Currently sitting in `public/`, the web-servable static-assets folder — not an appropriate location for an internal policy document regardless of governance status. |
...
| CARE_INQUIRY_ARCHITECHTURE.md | `serve-os/` root (untracked) | Architecture | Architecture | Yes | `docs/architecture/` | Needs Migration | Diagrams care-inquiry source channels. Untracked and outside `docs/` — same migration pattern as the Operational Readiness Roadmap. |
...
- **Operational Readiness & Audit Preparedness Roadmap** — highest priority. This is the program's founding charter and currently sits untracked in `serve-os/documents/`, at real risk of being lost or diverging from the work it inspired.
...
- **CARE_INQUIRY_ARCHITECHTURE.md** — already in the repository but untracked and outside `docs/`; a low-effort, high-value migration.
