...
**Origin:** This module was seeded from the organization's existing working draft, `Serve_Background_Eligibility_Policy_v0.1_Draft.docx` (retained at `docs/organizational-knowledge/archive/Serve_Background_Eligibility_Policy_v0.1_Draft.docx` in this repository — moved out of `public/`, the web-servable static-assets folder, on 2026-07-12 since an internal draft policy document should never be reachable unauthenticated), which had already established the four-classification structure, an initial offense taxonomy, a six-step review workflow outline, and a set of anticipated Serve OS decision fields. This module formalizes, extends, and structures that draft into the full governance framework format — the substance of the original draft's classification boundaries and offense examples was preserved rather than rewritten.
